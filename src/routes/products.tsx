...
/**
 * `page` is optional with NO default, deliberately.
 *
 * A default here does not just seed a value — TanStack Router re-stringifies the
 * validated search and rewrites the address bar when it differs, so
 * `.default(1)` turned a plain visit into `?page=1` before the shopper had paged
 * anything. `.catch` still repairs a hand-edited or stale value; it only fires
 * when a value is present and invalid. Same rule the checkout route follows for
 * `?step=`: never write the default.
 */
...
function Catalog() {
  ...
  // Filter changes reset to page one by REMOVING the param, not by writing 1 —
  // otherwise clearing a filter leaves `?page=1` behind.
      ...
      {query.data && query.data.items.length > 0 ? (
        <>
          ...
          <Pagination
            ...
            onChange={(next) => {
              // Paging back to the first page clears the param too.
              void navigate({
