...
declare module "axios" {
  export interface AxiosRequestConfig {
    /**
     * Background pollers (cart/notification badges) set this so a failed
     * silent refresh does NOT redirect to /login. They still refresh —
     * otherwise an idle tab's polls would 401 forever once the access token
     * expires.
     */
    skipAuthRefresh?: boolean;
    /** Internal marker: this request already went through one 401 retry. */
    _retried?: boolean;
  }
}

/** Extra fields the response interceptor attaches to rejected errors. */
export interface EnrichedAxiosError extends AxiosError {
  isRateLimit?: boolean;
  apiError?: { code?: string; message?: string };
}

/**
 * Axios instance configured for the Kong gateway.
 * Base URL from environment variables (baked at build time).
 */
...
apiClient.interceptors.response.use(
  ...
  async (error: unknown) => {
    ...
    const enriched = error as EnrichedAxiosError;

    if (enriched.response) {
      const { config, response } = enriched;
      ...
      if (raw && typeof raw === "object") {
        ...
        enriched.message = nested.message || "An error occurred";
        enriched.apiError = nested;
      } else {
        enriched.message = (typeof raw === "string" && raw) || "An error occurred";
      }
    } else if (enriched.request) {
      // Network error
      enriched.message = "Network error. Please check your connection.";
    }
